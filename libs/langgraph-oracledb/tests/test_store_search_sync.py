# type: ignore

import uuid
from contextlib import contextmanager

import oracledb
import pytest
from langgraph.store.base import SearchItem

from langgraph_oracledb.store.oracle import OracleStore
from tests.conftest import (
    DEFAULT_CONNECTION_INFO,
    create_connection_string,
    skip_if_no_oracle,
)


def _get_conn_string():
    """Get Oracle connection string."""
    return create_connection_string(DEFAULT_CONNECTION_INFO)


@contextmanager
def _test_store(table_suffix: str | None = None):
    """Create a test store for SQL generation and validation."""
    # Generate unique table suffix to avoid collisions
    if table_suffix is None:
        table_suffix = f"test_{uuid.uuid4().hex[:8]}"

    conn_string = _get_conn_string()

    with OracleStore.from_conn_string(conn_string, table_suffix=table_suffix) as store:
        store.setup()
        yield store
        store.teardown()


class TestStoreSearchSync:
    @pytest.fixture(autouse=True)
    def run_after_each_test(self):
        yield
        conn_string = _get_conn_string()
        with oracledb.connect(conn_string) as conn:
            with conn.cursor() as cur:
                cur.execute("drop table if exists STORE_CONFIGS purge")

    @skip_if_no_oracle()
    def test_store_search_basic_operations(self):
        """Test basic store search operations with actual data."""
        with _test_store() as store:
            # Put some test data
            store.put(("users",), "user1", {"name": "Alice", "age": 30, "active": True})
            store.put(("users",), "user2", {"name": "Bob", "age": 25, "active": False})
            store.put(
                ("users",), "user3", {"name": "Charlie", "age": 35, "active": True}
            )
            store.put(("products",), "prod1", {"name": "Widget", "price": 19.99})
            store.put(("products",), "prod2", {"name": "Gadget", "price": 29.99})

            # Test basic search without filters
            results = store.search(("users",))
            assert len(results) == 3
            assert all(isinstance(r, SearchItem) for r in results)

            # Test search with namespace prefix (positional argument)
            results = store.search(("users",))
            assert len(results) == 3

            results = store.search(("products",))
            assert len(results) == 2

            # Test search with limit and offset
            results = store.search(("users",), limit=2, offset=0)
            assert len(results) == 2

            results = store.search(("users",), limit=2, offset=2)
            assert len(results) == 1

    @skip_if_no_oracle()
    def test_store_search_with_filters(self):
        """Test store search with various filter conditions."""
        with _test_store() as store:
            # Put test data
            store.put(
                ("docs",),
                "doc1",
                {"title": "Python Guide", "views": 1000, "rating": 4.5},
            )
            store.put(
                ("docs",),
                "doc2",
                {"title": "Java Tutorial", "views": 500, "rating": 3.8},
            )
            store.put(
                ("docs",),
                "doc3",
                {"title": "Go Handbook", "views": 1500, "rating": 4.2},
            )
            store.put(
                ("docs",), "doc4", {"title": "Rust Manual", "views": 800, "rating": 4.7}
            )

            # Test equality filter
            results = store.search(("docs",), filter={"title": "Python Guide"})
            assert len(results) == 1
            assert results[0].value["title"] == "Python Guide"

            # Test greater than filter
            results = store.search(("docs",), filter={"views": {"$gt": 900}})
            assert len(results) == 2
            assert all(r.value["views"] > 900 for r in results)

            # Test greater than or equal filter (handle Decimal from Oracle)
            results = store.search(("docs",), filter={"rating": {"$gte": 4.2}})
            assert len(results) == 3
            assert all(float(r.value["rating"]) >= 4.2 for r in results)

            # Test less than filter
            results = store.search(("docs",), filter={"views": {"$lt": 1000}})
            assert len(results) == 2
            assert all(r.value["views"] < 1000 for r in results)

            # Test not equal filter
            results = store.search(("docs",), filter={"title": {"$ne": "Python Guide"}})
            assert len(results) == 3
            assert all(r.value["title"] != "Python Guide" for r in results)

            # Test multiple filters combined (handle Decimal from Oracle)
            results = store.search(
                ("docs",), filter={"views": {"$gte": 800}, "rating": {"$gt": 4.0}}
            )
            assert len(results) == 3
            for r in results:
                assert int(r.value["views"]) >= 800
                assert float(r.value["rating"]) > 4.0

    @skip_if_no_oracle()
    def test_store_search_complex_filters(self):
        """Test store search with complex nested filter conditions."""
        with _test_store() as store:
            # Put test data with nested structures
            store.put(
                ("items",),
                "item1",
                {
                    "type": "book",
                    "metadata": {"author": "Alice", "year": 2020},
                    "stats": {"views": 1000, "likes": 50},
                },
            )
            store.put(
                ("items",),
                "item2",
                {
                    "type": "article",
                    "metadata": {"author": "Bob", "year": 2021},
                    "stats": {"views": 500, "likes": 25},
                },
            )
            store.put(
                ("items",),
                "item3",
                {
                    "type": "book",
                    "metadata": {"author": "Charlie", "year": 2019},
                    "stats": {"views": 1500, "likes": 75},
                },
            )

            # Test filter on top-level field
            results = store.search(("items",), filter={"type": "book"})
            assert len(results) == 2

            # Test combination of filters
            results = store.search(
                ("items",), filter={"type": "book", "stats.views": {"$gte": 1000}}
            )
            # Note: nested path filtering might not work directly without JSON path support
            # This tests the basic filter structure generation

    @skip_if_no_oracle()
    def test_store_search_special_characters(self):
        """Test store search with special characters in values."""
        with _test_store() as store:
            # Put data with special characters
            store.put(("test",), "key1", {"desc": "Hello 'World'"})
            store.put(("test",), "key2", {"desc": 'Test "quotes"'})
            store.put(("test",), "key3", {"desc": "Path/with/slashes"})
            store.put(("test",), "key4", {"desc": "Semi;colon"})

            # Search for items with special characters
            results = store.search(("test",), filter={"desc": "Hello 'World'"})
            assert len(results) == 1
            assert results[0].value["desc"] == "Hello 'World'"

            results = store.search(("test",), filter={"desc": 'Test "quotes"'})
            assert len(results) == 1

            # Test that special characters don't break SQL
            results = store.search(
                ("test",), filter={"desc": {"$ne": "'; DROP TABLE --"}}
            )
            assert len(results) == 4  # Should return all items safely

    @skip_if_no_oracle()
    def test_store_search_empty_results(self):
        """Test store search behavior with no matching results."""
        with _test_store() as store:
            # Put some data
            store.put(("data",), "item1", {"value": 100})
            store.put(("data",), "item2", {"value": 200})

            # Search for non-existent namespace
            results = store.search(("nonexistent",))
            assert results == []

            # Search with filter that matches nothing
            results = store.search(("data",), filter={"value": {"$gt": 1000}})
            assert results == []

            # Search with impossible filter (no values should be both > 150 AND < 150)
            # Note: Since Oracle doesn't support combining multiple conditions on same field in a single filter,
            # we'll use a different approach - search for value that doesn't exist
            results = store.search(("data",), filter={"value": 999})
            assert results == []

    @skip_if_no_oracle()
    def test_store_search_pagination(self):
        """Test store search pagination with offset and limit."""
        with _test_store() as store:
            # Put multiple items
            for i in range(20):
                store.put(
                    ("items",), f"item{i:02d}", {"index": i, "value": f"value_{i}"}
                )

            # Test first page
            page1 = store.search(("items",), limit=5, offset=0)
            assert len(page1) == 5

            # Test second page
            page2 = store.search(("items",), limit=5, offset=5)
            assert len(page2) == 5

            # Ensure no overlap between pages
            page1_keys = {r.key for r in page1}
            page2_keys = {r.key for r in page2}
            assert page1_keys.isdisjoint(page2_keys)

            # Test last page
            last_page = store.search(("items",), limit=5, offset=15)
            assert len(last_page) == 5

            # Test offset beyond data
            empty_page = store.search(("items",), limit=5, offset=100)
            assert empty_page == []

    @skip_if_no_oracle()
    def test_store_search_namespace_prefix(self):
        """Test store search with namespace prefix patterns."""
        with _test_store() as store:
            # Create hierarchical namespace structure
            store.put(("docs",), "d1", {"type": "root"})
            store.put(("docs", "python"), "d2", {"type": "python"})
            store.put(("docs", "python", "tutorial"), "d3", {"type": "tutorial"})
            store.put(("docs", "java"), "d4", {"type": "java"})
            store.put(("docs", "java", "guide"), "d5", {"type": "guide"})
            store.put(("other",), "o1", {"type": "other"})

            # Search with namespace prefix (this searches ALL under the prefix)
            results = store.search(("docs",))
            assert len(results) == 5  # All items under "docs" prefix

            # Test that exact namespace is included in prefix search
            root_items = [r for r in results if r.namespace == ("docs",)]
            assert len(root_items) == 1
            assert root_items[0].value["type"] == "root"

            results = store.search(("docs", "python"))
            assert len(results) == 2  # Items under "docs.python" prefix

            # Search with filter and namespace prefix
            results = store.search(("docs",), filter={"type": "tutorial"})
            assert len(results) == 1
            assert results[0].value["type"] == "tutorial"

    @skip_if_no_oracle()
    def test_store_search_with_ttl(self):
        """Test store search with TTL (time-to-live) items."""
        with _test_store() as store:
            # Put items with different TTLs
            store.put(("temp",), "t1", {"data": "expires_soon"}, ttl=1)  # 1 minute
            store.put(("temp",), "t2", {"data": "expires_later"}, ttl=60)  # 60 minutes
            store.put(("temp",), "t3", {"data": "permanent"})  # No TTL

            # All items should be searchable immediately
            results = store.search(("temp",))
            assert len(results) == 3

            # Test with filter on TTL items
            results = store.search(("temp",), filter={"data": "permanent"})
            assert len(results) == 1
            assert results[0].value["data"] == "permanent"

    @skip_if_no_oracle()
    def test_store_batch_search_operations(self):
        """Test batch search operations."""
        with _test_store() as store:
            from langgraph.store.base import SearchOp

            # Put test data
            store.put(("users",), "u1", {"name": "Alice", "role": "admin"})
            store.put(("users",), "u2", {"name": "Bob", "role": "user"})
            store.put(("products",), "p1", {"name": "Widget", "price": 10})
            store.put(("products",), "p2", {"name": "Gadget", "price": 20})

            # Create batch search operations
            ops = [
                SearchOp(namespace_prefix=("users",), filter=None, limit=10, offset=0),
                SearchOp(
                    namespace_prefix=("products",),
                    filter={"price": {"$gte": 15}},
                    limit=10,
                    offset=0,
                ),
                SearchOp(
                    namespace_prefix=("users",),
                    filter={"role": "admin"},
                    limit=10,
                    offset=0,
                ),
            ]

            # Execute batch
            results = store.batch(ops)

            assert len(results) == 3
            assert len(results[0]) == 2  # All users
            assert len(results[1]) == 1  # Products with price >= 15
            assert len(results[2]) == 1  # Admin users
            assert results[2][0].value["name"] == "Alice"

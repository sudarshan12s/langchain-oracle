...
async def test_metadata_filtering_boolean_values(saver_name: str, test_data) -> None:
    ...
    async with _async_saver(saver_name) as saver:
        ...
        await saver.aput(config, checkpoint_1, metadata_true, {})
        config_2 = dict(config)
        config_2["configurable"]["checkpoint_id"] = checkpoint_2["id"]
        await saver.aput(config_2, checkpoint_2, metadata_false, {})

        # Search for True values
        results_true = [c async for c in saver.alist(None, filter={"active": True})]
        assert len(results_true) == 1
        assert results_true[0].metadata["active"] is True

        # Search for False values
        results_false = [c async for c in saver.alist(None, filter={"active": False})]
        assert len(results_false) == 1
        assert results_false[0].metadata["active"] is False

        # Search by multiple boolean fields
        results_both_true = [
            c async for c in saver.alist(None, filter={"active": True, "enabled": True})
        ]
...
@pytest.mark.parametrize("saver_name", ["base", "pool"])
async def test_metadata_filtering_numeric_values(saver_name: str, test_data) -> None:
    ...
    async with _async_saver(saver_name) as saver:
        ...
        await saver.aput(config, checkpoint_1, metadata_int, {})

        config_2 = dict(config)
        config_2["configurable"]["checkpoint_id"] = checkpoint_2["id"]
        await saver.aput(config_2, checkpoint_2, metadata_float, {})

        config_3 = dict(config)
        config_3["configurable"]["checkpoint_id"] = checkpoint_3["id"]
        await saver.aput(config_3, checkpoint_3, metadata_mixed, {})

        # Search for integer values
        results_int = [c async for c in saver.alist(None, filter={"count": 42})]
        assert len(results_int) == 1
        assert results_int[0].metadata["count"] == 42

        # Search for float values
        results_float = [c async for c in saver.alist(None, filter={"rate": 3.14})]
        assert len(results_float) == 1
        assert float(results_float[0].metadata["rate"]) == pytest.approx(3.14)

        # Search for mixed numeric values
        results_mixed = [
            c async for c in saver.alist(None, filter={"count": 100, "rate": 2.5})
        ]
        ...
        results_none = [c async for c in saver.alist(None, filter={"count": 999})]
        assert len(results_none) == 0


@pytest.mark.parametrize("saver_name", ["base", "pool"])
async def test_metadata_filtering_null_values(saver_name: str, test_data) -> None:
    ...
    async with _async_saver(saver_name) as saver:
        ...
        await saver.aput(config, checkpoint_1, metadata_with_null, {})

        config_2 = dict(config)
        config_2["configurable"]["checkpoint_id"] = checkpoint_2["id"]
        await saver.aput(config_2, checkpoint_2, metadata_without_null, {})

        # Search for None values
        results_null = [c async for c in saver.alist(None, filter={"optional": None})]
        assert len(results_null) == 1
        assert results_null[0].metadata["optional"] is None

        # Search for non-null values
        results_not_null = [
            c async for c in saver.alist(None, filter={"optional": "value"})
        ]
...
@pytest.mark.parametrize("saver_name", ["base", "pool"])
async def test_metadata_filtering_complex_objects(saver_name: str, test_data) -> None:
    ...
    async with _async_saver(saver_name) as saver:
        ...
        await saver.aput(config, checkpoint_1, metadata_dict, {})

        config_2 = dict(config)
        config_2["configurable"]["checkpoint_id"] = checkpoint_2["id"]
        await saver.aput(config_2, checkpoint_2, metadata_list, {})

        config_3 = dict(config)
        config_3["configurable"]["checkpoint_id"] = checkpoint_3["id"]
        await saver.aput(config_3, checkpoint_3, metadata_simple, {})

        # Search for exact dict match
        results_dict = [
            c async for c in saver.alist(None, filter={"config": complex_dict})
        ]
        ...
        results_list = [
            c async for c in saver.alist(None, filter={"items": complex_list})
        ]
        ...
        results_simple = [
            c async for c in saver.alist(None, filter={"config": simple_dict})
        ]
        ...
        results_none = [
            c async for c in saver.alist(None, filter={"config": non_matching})
        ]
        assert len(results_none) == 0


@pytest.mark.parametrize("saver_name", ["base", "pool"])
async def test_before_parameter_filtering(saver_name: str, test_data) -> None:
    ...
    async with _async_saver(saver_name) as saver:
        ...
        await saver.aput(config_1, checkpoint_1, metadata, {})
        await saver.aput(config_2, checkpoint_2, metadata, {})
        await saver.aput(config_3, checkpoint_3, metadata, {})
        ...
        results_before = [c async for c in saver.alist(config, before=before_config)]
        ...
        results_all = [c async for c in saver.alist(config)]
        assert len(results_all) == 3


@pytest.mark.parametrize("saver_name", ["base", "pool"])
async def test_checkpoint_id_config_filtering(saver_name: str, test_data) -> None:
    ...
    async with _async_saver(saver_name) as saver:
        ...
        results = [c async for c in saver.alist(search_config)]
        ...
        results_none = [c async for c in saver.alist(search_config_none)]
        assert len(results_none) == 0


@pytest.mark.parametrize("saver_name", ["base", "pool"])
async def test_combined_filtering_scenarios(saver_name: str, test_data) -> None:
    ...
    async with _async_saver(saver_name) as saver:
        ...
        await saver.aput(config, checkpoint_1, metadata_1, {})
        ...
        config_3 = dict(config)
        config_3["configurable"]["checkpoint_id"] = checkpoint_3["id"]
        await saver.aput(config_3, checkpoint_3, metadata_3, {})
        ...
        results_null = [
            c async for c in saver.alist(None, filter={"priority": 1, "optional": None})
        ]
        ...
        results_config_meta = [
            c
            async for c in saver.alist(
                config_filter, filter={"status": "active", "enabled": True}
            )
        ]
